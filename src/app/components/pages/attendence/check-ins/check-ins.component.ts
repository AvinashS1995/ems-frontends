...
export class CheckInsComponent {
  ...
  onCheckInsSubmit() {
    ...
    if (this.checkInsForm.valid) {
      this.apiService
        .postApiCall(API_ENDPOINTS.SERVICE_SEND_CHECK_INS_OTP, payload)
        .subscribe({
          next: (res: any) => {
            console.log(
              `${API_ENDPOINTS.SERVICE_SEND_CHECK_INS_OTP} Response : `,
              res
            );
  ...
  onVerifyCheckInsOtp() {
    ...
    if (this.checkInsForm.valid) {
      this.apiService
        .postApiCall(API_ENDPOINTS.SERVICE_VERIFY_CHECK_INS_OTP, payload)
        .subscribe({
          next: (res: any) => {
            console.log(
              `${API_ENDPOINTS.SERVICE_VERIFY_CHECK_INS_OTP} Response : `,
              res
            );
  ...
  loadUserCheckIns() {
    if (typeof window !== 'undefined') {
      this.hasCheckedIn =
        localStorage.getItem('checkIns') || sessionStorage.getItem('checkIns');
